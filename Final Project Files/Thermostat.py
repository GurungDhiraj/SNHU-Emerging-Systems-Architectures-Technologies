...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        ...
        # Update thermostat lights on entering heat state
        self.updateLights()
    ...
    def on_exit_heat(self):
        # Update thermostat lights on exitting heat state
        redLight.off()
    ...
    def on_enter_cool(self):
        ##
       # Update thermostat lights on entering cool state
        self.updateLights()
    ...
    def on_enter_off(self):
        #Turn both lights off for off state
        blueLight.off()
        redLight.off()
    ...
    def processTempStateButton(self):
        ...
        # changes the state of thermostat***
        self.send("cycle")
    ...
    def processTempIncButton(self):
        ...
        # Increase Setpoint temperature by 1
        self.setPoint = self.setPoint +1

        # Update the status of lights after increase
        self.updateLights()
    ...
    def processTempDecButton(self):
        ...
        # Decrease Setpoint temperature by 1
        self.setPoint = self.setPoint - 1

        # Update the status of lights after decrease
        self.updateLights()
    ...
    def updateLights(self):
        ...
        # Heating mode for thermostat
        if(self.current_state.id == 'heat'):
            # Current temperature is less than set temperature
            if (temp < self.setPoint):
                # Red light fades in and out
                redLight.pulse()

            # Current temperature is greater than or equal to set temperature
            else:
                # Solid red light
                redLight.on()

        # Cooling mode for thermostat
        elif(self.current_state.id == 'cool'):
            # Current temperature is greater than set temperature
            if (temp > self.setPoint):
                # Blue light fades in and out
                blueLight.pulse()

            # Current temperature is less than or equal to set temperature
            else:
                # Solid blue light
                blueLight.on()
        
        # OFF mode for thermostat
        else:
            pass # b/c the lights are already off in above code
    ...
    def setupSerialOutput(self):
        ...
        # Comma delimited string that will output the current thermostat state, temperature, and setpoint (both in Fahrenheit)
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = current_time.now().strftime('%b %d  %H:%M:%S\n')
    
            ## Setup Display Line 2
            if(altCounter < 6):
                # Display the current temperature (T) in degrees Fahrenheit
                lcd_line_2 = f"T:{self.getFahrenheit():0.1f}F"
    
                altCounter = altCounter + 1
            else:
            
                # Display current thermostat state (ST) and temperature setpoint SP (in Fahrenheit)
                lcd_line_2 = f"ST:{tsm.current_state.id}, SP:{self.setPoint}F"
            ...
            if((counter % 30) == 0):
                ...
                # Send current state information to TemperatureServer
...
## Configure our red button to use GPIO 24 and to execute
## the method to cycle the thermostat when pressed.
redButton = Button(24)

# Change thermostat state when pressed
redButton.when_pressed = tsm.processTempStateButton

## Configure our yellow button to use GPIO 12 and to execute
## the function to decrease the setpoint by a degree.
yellowButton = Button(12)

# Decrease setpoint by 1 degree when pressed
yellowButton.when_pressed = tsm.processTempDecButton

## Configure our gray button to use GPIO 25 and to execute
## the function to increase the setpoint by a degree.
grayButton = Button(25)

# Increase setpoint by 1 degree when pressed
grayButton.when_pressed = tsm.processTempIncButton
